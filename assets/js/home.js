document.addEventListener("DOMContentLoaded", () => {
  ...
  const imagesCarousel = document.querySelectorAll(
    ".carrousel_vinos .carrousel-image"
  );
  ...
  setInterval(() => {
    ...
    currentIndexBackground =
      (currentIndexBackground + 1) % imagesBackground.length; // Cicla entre las imágenes
  }, 3000); // Cambia cada 3 segundos

  // Carrusel de postres
  const postresImages = document.querySelectorAll(
    ".imagenes_postres .carrousel-postre-image"
  );
  let postreIndex = 0;
...
function openModal() {
  ...
  if (typeof content === "string" && content.trim() !== "") {
    ...
    modalBody.innerHTML = "<p>Cargando carta...</p>";

    Promise.all([
      fetch("/brunet/views/admin/functions/getCarta.php").then((res) =>
        res.json()
      ),
      fetch("/brunet/views/admin/functions/getCategorias.php").then((res) =>
        res.json()
      ),
    ])
      .then(([platos, categorias]) => {
        modalBody.innerHTML = "<h2>Carta del Restaurante</h2>";

        categorias.forEach((cat) => {
          const platosDeCategoria = platos.filter(
            (plato) => plato.categoria_id == cat.id
          );

          if (platosDeCategoria.length === 0 || (cat.id >= 13 && cat.id <= 16))
            return;
      ...
      .catch((err) => {
        ...
        modalBody.innerHTML =
          '<p style="color: red;">Error al cargar la carta.</p>';
      });
  }
}

function openModalSug() {
  const modal = document.getElementById("modal");
  const modalContent = modal.querySelector(".modal-content");
  const modalBody = document.getElementById("modal-body");
  console.log("hola");
  ...
  if (typeof content === "string" && content.trim() !== "") {
    ...
    Promise.all([
      ...
      .then(([platos, categorias]) => {
        modalBody.innerHTML = "<h2>Sugerencias del Chef</h2>";

        categorias.forEach((cat) => {
          const platosDeCategoria = platos.filter(
            (plato) =>
              plato.categoria_id == cat.id && Number(plato.sugerencia) === 1
          );
...
function openModalVinos() {
  ...
  modalBody.innerHTML = "<p>Cargando carta de vinos...</p>";
  ...
  Promise.all([
    ...
    .then(([platos, categorias]) => {
      modalBody.innerHTML = "<h2>Carta de Vinos</h2>";

      const botonesContainer = document.createElement("div");
      botonesContainer.classList.add("menu-buttons");

      const vinosContainer = document.createElement("div");
      vinosContainer.classList.add("vino-list");

      // Filtrar categorías de vinos (IDs 13 al 16)
      const categoriasVino = categorias.filter(
        (cat) => cat.id >= 13 && cat.id <= 16
      );

      let hayBotones = false;

      categoriasVino.forEach((cat) => {
        const platosDeCategoria = platos.filter(
          (plato) => plato.categoria_id == cat.id
        );

        // Si no hay vinos de esta categoría, no crear botón
        if (platosDeCategoria.length === 0) return;
        ...
        boton.classList.add("vino-btn");

        boton.onclick = () => {
          vinosContainer.innerHTML = ""; // Limpiar contenido anterior

          const h3 = document.createElement("h3");
          h3.textContent = cat.nombre;
          vinosContainer.appendChild(h3);
          ...
          vinosContainer.appendChild(div);
        };
      ...
      if (hayBotones) {
        modalBody.appendChild(botonesContainer);
        modalBody.appendChild(vinosContainer);
      } else {
        modalBody.innerHTML = "<p>No hay vinos disponibles actualmente.</p>";
      }
    })
    .catch((err) => {
      console.error("Error:", err);
...
function openModalDiario() {
  ...
  modalBody.innerHTML = "<p>Cargando menú diario...</p>";

  Promise.all([
    fetch("/brunet/views/admin/functions/getCarta.php").then((res) => res.json()),
    fetch("/brunet/views/admin/functions/getMenus.php").then((res) => res.json()),
    fetch("/brunet/views/admin/functions/getMenuPlato.php").then((res) => res.json()),
  ])
    .then(([platos, menus, platoMenu]) => {
      const menuDiario = menus.find((menu) => menu.categoria === "diario");
      ...
      tituloGeneral.textContent = "Menú Diario";
      modalBody.appendChild(tituloGeneral);

      if (!menuDiario) {
        const mensaje = document.createElement("p");
        mensaje.textContent = "No hay menú diario disponible actualmente.";
        modalBody.appendChild(mensaje);
        return;
      }

      const relaciones = platoMenu.filter((rel) => rel.menu_id == menuDiario.id);

      if (relaciones.length === 0) {
        const mensaje = document.createElement("p");
        mensaje.textContent = "Este menú no tiene platos asignados.";
        modalBody.appendChild(mensaje);
        return;
      }

      // Agregar título del menú diario
      const tituloMenu = document.createElement("h3");
      tituloMenu.textContent = menuDiario.nombre;
      modalBody.appendChild(tituloMenu);
      ...
      contenidoContainer.classList.add("menu-list");

      for (let cat = 1; cat <= 4; cat++) {
        const relacionesCat = relaciones.filter((r) => r.categoria == cat);
        if (relacionesCat.length === 0) continue;

        const platosCat = relacionesCat
          .map((r) => {
            const plato = platos.find((p) => p.id == r.plato_id);
            return plato ? { ...plato, comentario: r.comentario || "" } : null;
          })
          .filter((p) => p);

        if (platosCat.length === 0) continue;

        const h4 = document.createElement("h4");
        h4.textContent = categoriasTexto[cat];
        contenidoContainer.appendChild(h4);

        const div = document.createElement("div");
        div.classList.add("menu-section");

        platosCat.forEach((plato) => {
          const comentario = plato.comentario ? ` (${plato.comentario})` : "";
          const p = document.createElement("p");
          p.innerHTML = `<span>${plato.nombre}${comentario}</span>`;
          div.appendChild(p);
        });

        contenidoContainer.appendChild(div);
      }

      modalBody.appendChild(contenidoContainer);
    })
    .catch((err) => {
      modalBody.innerHTML =
        '<p style="color: red;">Error al cargar el menú:<br>' + err + "</p>";
    });
}
...
function openModalMenus() {
  ...
  modalBody.innerHTML = "<p>Cargando menús...</p>";

  Promise.all([
    fetch("/brunet/views/admin/functions/getCarta.php").then((res) =>
      res.json()
    ),
    fetch("/brunet/views/admin/functions/getMenus.php").then((res) =>
      res.json()
    ),
    fetch("/brunet/views/admin/functions/getMenuPlato.php").then((res) =>
      res.json()
    ),
  ])
    .then(([platos, menus, platoMenu]) => {
      modalBody.innerHTML = "<h2>Menús de Grupo</h2>";
      ...
      // 🔍 Filtrar menús por categoría "grupo"
      const menusDeGrupo = menus.filter((menu) => menu.categoria === "grupo");

      menusDeGrupo.forEach((menu) => {
        const relaciones = platoMenu.filter((rel) => rel.menu_id == menu.id);
        if (relaciones.length === 0) return;

        hayMenus = true;

        const boton = document.createElement("button");
        boton.classList.add("btn", "menu-btn");
        ...
        boton.onclick = () => {
          ...
          // Agrupar platos por categoría del menú
          for (let cat = 1; cat <= 4; cat++) {
            const relacionesCat = relaciones.filter((r) => r.categoria == cat);
            if (relacionesCat.length === 0) continue;
            ...
            platosCat.forEach((plato) => {
              const comentario = plato.comentario
                ? ` (${plato.comentario})`
                : "";
              const p = document.createElement("p");
              p.innerHTML = `<span>${plato.nombre}${comentario}</span>`;
      ...
      if (hayMenus) {
        ...
        modalBody.innerHTML =
          "<p>No hay menús de grupo disponibles actualmente.</p>";
      }
    })
    .catch((err) => {
      modalBody.innerHTML =
        '<p style="color: red;">Error al cargar los menús:<br>' + err + "</p>";
    });
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const containers = Array.from(
    document.querySelectorAll(".container_carrousel_imgs")
  );
  ...
  while (backgrounds.length < containers.length) {
    ...
    backgrounds.push(
      `/brunet/assets/media/img/postres/${postresImgs[postresIndex++]}`
    );
  }
  ...
  const columns = Array.from(
    document.querySelectorAll(".carrousel_platos .column_carrousel")
  );
  ...
  function animateColumns(scrollDelta, direction) {
    columns.forEach((col, idx) => {
      ...
      let next = current + factor * scrollDelta * 0.2;
